...
- v1.2.0：支持删除已发布文章，记录时间精确到分钟；作者验证在当前浏览器会话内复用凭证，关闭浏览器或退出即清除。
- v1.0.1：开发阶段方案，未发布，合并至 v1.2.0。
...
令牌仅保存在当前浏览器会话的 `sessionStorage`，不写入永久本地存储或仓库；退出/关闭浏览器会清除。作者页可以被访问，但 GitHub 在服务端强制校验仓库写入权限。前端还检查账户是 `albertyizhan`；此检查不能替代 GitHub 权限，勿给其他账户仓库写权限、不要分享令牌。更换仓库、账户或发布分支时同步修改 `blog.js`。
